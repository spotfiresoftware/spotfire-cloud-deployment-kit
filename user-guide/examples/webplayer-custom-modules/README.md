...
bundled as sdn or spk files and must be unpacked to a persistent volume
before they can be used by the webplayer.

Creating custom webplayer images that include the custom modules is also
possible, but this example does not cover that. Using a persistent volume has
...
Steps 1-5 below are only needed if you want to unpack the modules to a persistent
...
3. Push the image to a Docker registry so it is available to your Kubernetes
   cluster.
   ```
...
6. To install spotfire-webplayer, use the `volumes.customModules.existingClaim`
   ...
   **Note:** spotfire/spotfire-webplayer assumes the helm repo is named spotfire.
   If you have named it something else, you must use that name instead.
