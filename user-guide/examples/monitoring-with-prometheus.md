...
3. Set up port-forwarding to access the Prometheus server pod:
   ```bash
...
(the address to access the Prometheus server within your cluster that was shown when you deployed the Prometheus Helm chart)
...
2. [Import a dashboard](https://grafana.com/docs/grafana/latest/dashboards/export-import/) using a previously exported dashboard as a JSON file, or by dashboard ID.
    - Start exploring with the [Spotfire Platform](monitoring/Spotfire-Platform-grafana-dashboard.json) dashboard example.
