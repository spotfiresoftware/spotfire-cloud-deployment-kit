...
The Spotfire Server:
- Facilitates configuration and administration of the Spotfire Platform.
- Provides access to external data sources.
- Manages the traffic through the Spotfire platform.
...
2. From the `<this-repo>/docker` folder, run `make spotfire-server` to build this image, or `make spotfire-server --dry-run` to just view the commands.
...
- `SPOTFIREDB_PASSWORD` - See **--password** in the [bootstrap command](https://docs.tibco.com/pub/spotfire_server/latest/doc/html/TIB_sfire_server_tsas_admin_help/server/topics/bootstrap.html) help.
   Example: *p4s5w0rd!*.
- `SPOTFIREDB_AUTO_UPGRADE` - Set to **Y** to automatically perform database upgraded.
...
- `SITE_NAME` - Define which site the server should belong tt. Default: `Default`.
